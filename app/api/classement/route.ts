...
const TRIS_AUTORISES: RankingSort[] = ["max_points", "victoires", "ratio"];
 ...
 *   ?tri=max_points|victoires|ratio
 *   ?limit=50
 */
export async function GET(request: Request): Promise<Response> {
  try {
    const limit = readIntParam(request, "limit", 50);
    const tri = new URL(request.url).searchParams.get("tri") ?? "max_points";
